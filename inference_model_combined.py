...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb  2 19:48:52 2022
...
model_folder1='attempt/ant/model/'
model_folder2='attempt/post/model/'
model_vis='attempt/combined/vis/'
log_train = 'training_log.txt'       # name of files with saved training loss and perplexity
log_val = 'validation_log.txt'

bleu = 'bleu.txt'
gt_csv='combined/fine_scores/combined_outs.csv'
import csv
...
if flag==1:
    ...
    def visualize_attention(orig_image, outputs, atten_weights,outputs2, atten_weights2, name, ground):
        ...
        atten_weights2= atten_weights2.cpu().detach().numpy().copy()
        atten_weights=atten_weights+atten_weights2
        
        words=outputs.max(dim=-1).indices.cpu().detach().numpy().copy()
        words2=outputs2.max(dim=-1).indices.cpu().detach().numpy().copy()
        words=words+words2
        words=words[:-1]
        ...
        for i in range(len(words)):
            ...
            ax = fig.add_subplot(1, len_tokens, i+1)
        ...
        return dist
    # Build data loader.
    data = get_k_fold_loader(dir_name, binary_dir,label_csv_name,label_fine,batch_size, level1, k=10, test=False)
    print('Data Loaded')
    ...
    encoder2 = EncoderCNN()
    decoder2 = DecoderRNN(num_features = num_features, 
                             embedding_dim = embed_size, 
                             hidden_dim = hidden_size, 
                             vocab_size = vocab_size,
                             device=device)
    ...
    for curr_fold, (train_batch_loader, valid_batch_loader) in enumerate(data):
            ...
            encoder_file = model_folder1+ level1+'_encoder-'+str(curr_fold)+'_4.pkl' 
            decoder_file = model_folder1+ level1+'_decoder-'+str(curr_fold)+'_4.pkl' 
            ...
            encoder_file2 = model_folder2+ level2+'_encoder-'+str(curr_fold)+'_4.pkl' 
            decoder_file2 = model_folder2+ level2+'_decoder-'+str(curr_fold)+'_4.pkl' 
            
            # Load the trained weights.
            encoder2.load_state_dict(torch.load(encoder_file2))
            decoder2.load_state_dict(torch.load(decoder_file2))
            ...
            if flag2==1:
                ...
                for i_step,(val_images, labels,val_captions, name) in enumerate(val_data_loader):
                  ...
                  # evaluation of encoder and decoder
                          ...
                          features_val2 = encoder2(val_images).to(device)
                          outputs_val2, atten_weights_val2 = decoder(captions= val_captions,
                                                   features = features_val2)
                          outputs2= outputs_val2.max(dim=-1).indices.cpu().detach().numpy().copy()
                          ...
                          for n in range(len(val_images)):
                              ...
                              atten_weights2=atten_weights_val2[n,:,:]
                              outputs2=outputs_val2[n]
                              ground=gt_dic[name[n]]
                              
                              
                              dist=visualize_attention(orig_image,outputs, atten_weights,outputs2, atten_weights2, name[n], ground)
                              gt_distance[name[n]]=dist
